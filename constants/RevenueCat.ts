...
export const REVENUECAT_CONFIG = {
   ...
   */
  IOS_API_KEY: 'your_ios_api_key_here', // e.g., 'appl_abcdef1234567890'
  ANDROID_API_KEY: 'your_android_api_key_here', // e.g., 'goog_abcdef1234567890'
  WEB_API_KEY: 'your_web_api_key_here', // e.g., 'strp_abcdef1234567890' (for Web Billing)
...
export const getPlatformApiKey = (): string => {
  switch (Platform.OS) {
    ...
    default:
      console.warn(`Unsupported platform: ${Platform.OS}. Falling back to iOS API key.`);
      return REVENUECAT_CONFIG.IOS_API_KEY;
  }
};
...
export const validateRevenueCatConfig = (): boolean => {
  ...
  if (!apiKey || apiKey.includes('your_') || apiKey.includes('_here')) {
    console.error('⚠️ RevenueCat API key not configured for platform:', Platform.OS);
    console.error('Please update constants/RevenueCat.ts with your actual API keys');
    console.error('Get your API keys from: https://app.revenuecat.com/projects/[your-project]/api-keys');
    return false;
  }
  
  if (!REVENUECAT_CONFIG.ENTITLEMENT_ID || REVENUECAT_CONFIG.ENTITLEMENT_ID === 'premium') {
    console.warn('⚠️ Using default entitlement ID. Consider updating to match your RevenueCat configuration.');
  }
